...
def fetch_audio(t_id):
    return session.execute(text(f"select input::jsonb->'files'->>'recording',output::jsonb->'data' from microtask where task_id = '{t_id[0]}' and input::jsonb->'chain'->'workerId' = '{w_id[0]}' and output is not null and  (output::text like '%\"decision\":\"accept\"%' or  output::text like '%\"decision\":\"excellent\"%') limit 2;")).fetchall()
